...
function resolveAuthor(post: FeedPost, me: CurrentUser | null): AuthorInfo | null {
  ...
  // "me" = the signed-in user posting as themselves (openings via PostFlow)
  if (id === "me") {
    return me
      ? { name: me.name, seed: 99, href: "/profile", typeLabel: "You", square: false }
      : null;
  }
...
function SubEmbed({
  ...
  authorName: string;
  /** the viewer posted this opening — swap the respond CTA for status + SOS */
  own?: boolean;
}) {
  ...
  return (
    <div className="mt-3 rounded-xl border border-amber-500/40 bg-amber-500/[0.06] p-3.5 shadow-[0_0_28px_-10px_rgba(255,138,61,0.5)]">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5">
            ...
            <span className="text-text-faint">·</span>
            {sub.payout !== undefined ? (
              <Mono className="text-[11px] font-bold text-amber-300">
                ${sub.payout} guaranteed
              </Mono>
            ) : (
              // fees are private — the number lives in the 1:1 offer thread
              <Mono className="inline-flex items-center gap-1 text-[11px] text-cyan-300">
                <LockIcon size={11} />
                Fee in the DM
              </Mono>
            )}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        {own ? (
          <>
            <Mono className="inline-flex items-center gap-1.5 text-[11px] text-cyan-300">
              <span className="blink h-1.5 w-1.5 rounded-full bg-cyan-400" />
              Your opening — replies land in Chats
            </Mono>
            <Button
              size="sm"
              variant="ghost"
              className="ml-auto"
              onClick={() => navigate(`/?sos=open&role=${sub.instrument}`)}
            >
              Find subs →
            </Button>
          </>
  ...
  return (
    <Card className="p-4">
      ...
      {post.kind === "need-sub" && post.subFor && (
        <SubEmbed post={post} sub={post.subFor} authorName={author.name} own={post.own} />
      )}
